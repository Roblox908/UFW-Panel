...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${activeBackend.url}/enable`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error("Failed to enable UFW via backend:", errorDetails);
      return NextResponse.json({ error: 'Failed to enable UFW', details: errorDetails }, { status: response.status });
    }
    return NextResponse.json({ message: 'UFW enabled successfully via backend' }, { status: response.status }); 
