...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${activeBackend.url}/rules/deny`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error(`Failed to add deny rule "${rule}" via backend:`, errorDetails);
      return NextResponse.json({ error: `Failed to add deny rule "${rule}"`, details: errorDetails }, { status: response.status });
    }

    return NextResponse.json({ message: `Rule "${rule}" denied successfully via backend` }, { status: response.status }); 

  } catch (error: any) {
    console.error(`Error adding deny rule "${rule}":`, error);
