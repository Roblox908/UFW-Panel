...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${activeBackend.url}/rules/allow/ip`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error(`Failed to add allow IP rule for "${ip_address}" via backend:`, errorDetails);
      return NextResponse.json({ error: `Failed to add allow IP rule for "${ip_address}"`, details: errorDetails }, { status: response.status });
    }

    return NextResponse.json({ message: `IP rule for "${ip_address}" allowed successfully via backend` }, { status: response.status }); // Forward backend status

  } catch (error: any) {
    console.error(`Error adding allow IP rule for "${ip_address}":`, error);
